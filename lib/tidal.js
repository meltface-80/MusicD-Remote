...
// Reading TIDAL's base64 playback manifest. Declared up here, above its only
// caller far below: a const has no hoisting, and this project keeps a pre-flight
// step for exactly that class of startup crash.
const TMAN = require("./tidal-manifest");
...
// plugin's approach.
/*
 * The user's favourite albums — ALL of them.
 *
 * This was one call with `limit: 5000` and no paging, which is a cap wearing a
 * limit's clothes: a library past it came back truncated, with no error and no
 * way for the caller to tell. TIDAL states totalNumberOfItems in the same
 * response and nothing read it. It pages until the total is reached now.
 *
 * The guard is a stop against a server that never advances, not a library size
 * limit — it is high enough that reaching it means something is wrong, and the
 * caller is told when it is reached rather than handed a short list.
 */
async function getFavoriteAlbums(token, cc, userId, opts) {
  return (await getFavoriteAlbumsAll(token, cc, userId, opts)).items;
}

const FAV_PAGE = 1000, FAV_MAX_PAGES = 200;   // 200,000 albums before the guard

async function getFavoriteAlbumsAll(token, cc, userId, opts) {
  if (!userId) throw new Error("userId required");
  const o = opts || {};
  const onPage = typeof o.onPage === "function" ? o.onPage : null;
  let items = [], total = 0, truncated = false, page = 0;
  for (; page < FAV_MAX_PAGES; page++) {
    const r = await tidalGet("/users/" + encodeURIComponent(userId) + "/favorites/albums", {
      limit: FAV_PAGE, offset: items.length, countryCode: cc
    }, token);
    const sec = pagedSection(r);
    if (sec.total) total = sec.total;
    if (!sec.items.length) break;
    items = items.concat(sec.items);
    if (onPage) onPage(items.length, total);
    if (sec.items.length < FAV_PAGE) break;
    if (total && items.length >= total) break;
  }
  if (page >= FAV_MAX_PAGES) truncated = true;
...
/*
 * One album's track list — ids, titles and durations, which is everything the
 * waveform's track matcher needs.
 *
 * TIDAL reports `duration` in whole seconds, the same unit Roon uses, which is
 * what makes the duration gate a straight comparison rather than a conversion
 * with its own rounding to argue about.
 */
async function getAlbumTracks(token, cc, albumId, timeoutMs) {
  if (!albumId) return null;
  let r;
  try {
    r = await tidalRequest("GET", "/albums/" + encodeURIComponent(albumId) + "/tracks",
                           { countryCode: cc, limit: 100 }, token, null, timeoutMs || 12000);
  } catch (e) {
    return null;
  }
  const items = pagedSection(r).items || [];
  return {
    id: String(albumId),
    tracks: items.map((t) => ({
      id: t && t.id,
      title: (t && t.title) || "",
      duration: (t && Number.isFinite(t.duration)) ? t.duration : null,
      number: (t && t.trackNumber) || null,
    })).filter((t) => t.id),
  };
}

/*
 * A playable URL for one track, or null with a reason.
 *
 * THE ONLY CALL HERE THAT TOUCHES AUDIO. `assetpresentation=FULL` asks for the
 * track rather than a preview; the quality is deliberately the lowest that
 * still carries the shape, because the bytes are decoded into a thousand
 * loudness values and thrown away — and because the higher tiers come back in a
 * protected container this refuses to touch (see lib/tidal-manifest.js).
 */
async function getTrackStream(token, cc, trackId, opts) {
  const o = opts || {};
  if (!trackId) return { url: null, reason: "no track id" };
  let j;
  try {
    j = await tidalRequest("GET",
      "/tracks/" + encodeURIComponent(trackId) + "/playbackinfopostpaywall",
      { countryCode: cc,
        audioquality: o.quality || "LOW",
        playbackmode: "STREAM",
        assetpresentation: "FULL" },
      token, null, o.timeoutMs || 15000);
  } catch (e) {
    return { url: null, reason: "TIDAL refused the playback request: " + (e && e.message) };
  }
  return TMAN.streamUrlFrom(j);
}
...
module.exports = {
  ...
  getFavoriteAlbums, getFavoriteAlbumsAll, favoriteAlbum, unfavoriteAlbum,
  getAlbumTracks, getTrackStream,
  coverUrl
};
