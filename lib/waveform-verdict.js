...
function streamingVerdict(q, t) {
  ...
  return "this album is in neither service's FAVOURITES (" + qKnown + " Qobuz, " +
         tKnown + " TIDAL known) and nothing in either resembles it, so it is " +
         "genuinely absent rather than spelled differently. Roon never says " +
         "which service is playing, so favourite membership is the only signal " +
         "there is — playing from a search is not enough";
}
