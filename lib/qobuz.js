...
async function getAlbum(token, albumId, timeoutMs = 12000, appId) {
  ...
  let j;
  try {
    // `appId` travels with the token: a pasted credentials file's token is only
    // accepted alongside its own app_id, and this read happens on the same path
    // as the signed one. Omitted everywhere else, where the module's id is right.
...
async function getFileUrlResult(token, trackId, secret, opts) {
  ...
  try {
    ...
    return { url: null, reason: describeFileUrlError(e) };
  }
...
function describeFileUrlError(e) {
  ...
  if (status === 401) {
    return "Qobuz rejected the TOKEN (HTTP 401). The signing app_id is not the " +
           "one that minted this login token — they must come from the same app";
  }
  if (status === 404) return "Qobuz has no such track (HTTP 404)";
...
module.exports = { login, getFeaturedAlbums, searchCatalog, getArtist, favoriteAlbum, unfavoriteAlbum, getFavoriteAlbumIds, getFavoriteAlbums, getAlbum, getFileUrl, getFileUrlResult, describeFileUrlError, pickImage, md5Hex, APP_ID };
