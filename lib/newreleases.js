...
function readArtistAlbums(json) {
  ...
  for (const a of data) {
    if (!a) continue;
    if (String(a.record_type || "").toLowerCase() !== "album") continue;
 ...
 *
 * The last entry is the CDN path built from `md5_image`, and it is marked as
 * what it is: a pattern that is widely used but that could not be checked from
 * the machine this was written on, because the network policy there blocks
 * Deezer outright. It sits last on purpose — it is only reached when every
 * real field is absent, and if the pattern is wrong the image simply fails to
 * load and the row keeps the empty tile it would have had anyway. A guess that
 * can only turn "no cover" into "no cover" is a safe guess; one that could
 * turn a good cover into a broken one would not be.
 */
function coverOf(a) {
  const named = a.cover_medium || a.cover_big || a.cover_small || a.cover_xl || a.cover;
  if (named) return String(named);
  if (a.md5_image) {
    return "https://e-cdns-images.dzcdn.net/images/cover/" +
...
module.exports = {
  WANTED_PER_ARTIST, SEED_ARTISTS, EDITION_WORDS,
  normalize, titleKey, stripEdition, yearOf, dateMs,
  readArtistAlbums, coverOf, isReissue, owns, pickNewReleases, playedArtists,
};
