 ...
 *   per `stride` samples, which for the default 8 kHz decode is ~31 values a
 *   second — and resamples that down to the final bucket count once the length
 *   is known. A five-minute track holds ~9,400 intermediate values, so the
...
// One intermediate peak per this many input samples. At 8 kHz that is ~32ms,
// short enough that a snare still registers as its own peak.
const STRIDE = 256;
