...
```bash
wget https://github.com/meltface-80/Roon-Random-Albums-Extension/raw/main/roon-random-albums-v1.5.20-docker.tar.gz
tar -xzf roon-random-albums-v1.5.20-docker.tar.gz
cd roon-random-albums
docker build -t roon-random-albums:1.5.20 .
docker run -d \
  --name roon-random-albums \
  --restart unless-stopped \
  --network host \
  -v roon-random-albums-data:/app/data \
  roon-random-albums:1.5.20
```
...
```bash
docker run -d ... -e RRA_DEBUG=1 roon-random-albums:1.5.20
```
