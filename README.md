...
# Random Albums + Labels — a Roon extension v1.5.88
...
wget https://github.com/meltface-80/Roon-Random-Albums-Extension/releases/download/v1.5.88/roon-random-albums-v1.5.88-docker.tar.gz
tar -xzf roon-random-albums-v1.5.88-docker.tar.gz
docker build -t roon-random-albums:1.5.88 .
docker run -d \
...
```bash
docker run -d ... -e RRA_DEBUG=1 roon-random-albums:1.5.88
```
