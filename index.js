...
const { ZoneRelay, findLibrespot } = require("./lib/relay");
...
// Spotify Connect relay state
let audioinput    = null;  // RoonApiAudioInput service reference (set on pair)
let librespotPath = null;  // resolved librespot binary path (null = not installed)
const relays      = new Map(); // zone_id → ZoneRelay

// Detect librespot at startup so the UI can show status immediately.
(function detectLibrespot() {
  try { librespotPath = findLibrespot(); } catch {}
  if (DEBUG) console.log("[relay] librespot:", librespotPath || "not found");
})();
...
const roon = new RoonApi({
  ...
  core_paired: function (c) {
    core = c;
    audioinput = c.services.RoonApiAudioInput;
  ...
  core_unpaired: function () {
    core = null; audioinput = null; zones = {}; outputs = {};
    // Stop all active relays — Roon sessions are gone
    for (const relay of relays.values()) relay.stop().catch(() => {});
    relays.clear();
...
roon.init_services({
  required_services: [RoonApiTransport, RoonApiBrowse, RoonApiImage, RoonApiAudioInput],
...
const LABELS_CACHE_FILE = path.join(__dirname, "data", "labels-cache.json");

// key (normalize(title)||normalize(artist)) → label name string | "" (no label found)
const labelDiskCache = new Map();
let   _diskSaveTimer = null;

function loadLabelsDiskCache() {
  try {
    const raw  = fs.readFileSync(LABELS_CACHE_FILE, "utf8");
    const data = JSON.parse(raw);
    if (data && data.version === 1 && Array.isArray(data.entries)) {
      for (const e of data.entries) {
        // Only keep entries where a label was actually found.
        // Empty-string entries (failed lookups) are discarded so they get
        // rescanned — previous version used Qobuz which missed many albums.
        if (typeof e.key === "string" && e.label) labelDiskCache.set(e.key, e.label);
      }
      if (DEBUG) console.log("[labels] disk cache loaded:", labelDiskCache.size, "confirmed entries");
    }
  } catch (e) { /* file doesn't exist yet — that's fine */ }
}

function saveLabelsDiskCache() {
  try {
    const dir = path.dirname(LABELS_CACHE_FILE);
    if (!fs.existsSync(dir)) fs.mkdirSync(dir, { recursive: true });
    const entries = [];
    for (const [key, label] of labelDiskCache) entries.push({ key, label });
    fs.writeFileSync(LABELS_CACHE_FILE,
      JSON.stringify({ version: 1, savedAt: Date.now(), entries }));
  } catch (e) {
    if (DEBUG) console.error("[labels] disk save failed:", e.message);
  }
}

function scheduleDiskSave() {
  if (_diskSaveTimer) return;
  _diskSaveTimer = setTimeout(() => { _diskSaveTimer = null; saveLabelsDiskCache(); }, 10000);
}

// Load disk cache at startup (synchronous; small file, fast).
loadLabelsDiskCache();
...
// labelMbidCache: groupKey → label MBID (from MusicBrainz label-info.id)
// Persisted so we can hit Fan Art TV without re-scanning MusicBrainz.
const LABELS_MBID_FILE = path.join(__dirname, "data", "labels-mbid.json");
const labelMbidCache   = new Map();
let   _mbidSaveTimer   = null;

function loadLabelMbidCache() {
  try {
    const data = JSON.parse(fs.readFileSync(LABELS_MBID_FILE, "utf8"));
    if (data && Array.isArray(data.entries)) {
      for (const e of data.entries) {
        if (e.groupKey && e.mbid) labelMbidCache.set(e.groupKey, e.mbid);
      }
    }
    if (DEBUG) console.log("[labels] mbid cache loaded:", labelMbidCache.size, "entries");
  } catch (e) { /* file optional */ }
}

function saveLabelMbidCache() {
  try {
    const dir = path.dirname(LABELS_MBID_FILE);
    if (!fs.existsSync(dir)) fs.mkdirSync(dir, { recursive: true });
    const entries = [];
    for (const [groupKey, mbid] of labelMbidCache) entries.push({ groupKey, mbid });
    fs.writeFileSync(LABELS_MBID_FILE, JSON.stringify({ version: 1, savedAt: Date.now(), entries }));
  } catch (e) { if (DEBUG) console.error("[labels] mbid save failed:", e.message); }
}

function scheduleMbidSave() {
  if (_mbidSaveTimer) return;
  _mbidSaveTimer = setTimeout(() => { _mbidSaveTimer = null; saveLabelMbidCache(); }, 10000);
}

// labelLogoCache: groupKey → logoUrl (string) or null ("no logo found").
// undefined/absent = not yet queried.
const LABELS_LOGO_FILE = path.join(__dirname, "data", "labels-logo.json");
const labelLogoCache   = new Map();
let   _logoSaveTimer   = null;

function loadLabelLogoCache() {
  try {
    const data = JSON.parse(fs.readFileSync(LABELS_LOGO_FILE, "utf8"));
    if (data && Array.isArray(data.entries)) {
      for (const e of data.entries) {
        if (typeof e.groupKey === "string") labelLogoCache.set(e.groupKey, e.logoUrl || null);
      }
    }
    if (DEBUG) console.log("[labels] logo cache loaded:", labelLogoCache.size, "entries");
  } catch (e) { /* file optional */ }
}

function saveLabelLogoCache() {
  try {
    const dir = path.dirname(LABELS_LOGO_FILE);
    if (!fs.existsSync(dir)) fs.mkdirSync(dir, { recursive: true });
    const entries = [];
    for (const [groupKey, logoUrl] of labelLogoCache) entries.push({ groupKey, logoUrl });
    fs.writeFileSync(LABELS_LOGO_FILE, JSON.stringify({ version: 1, savedAt: Date.now(), entries }));
  } catch (e) { if (DEBUG) console.error("[labels] logo save failed:", e.message); }
}

function scheduleLogoSave() {
  if (_logoSaveTimer) return;
  _logoSaveTimer = setTimeout(() => { _logoSaveTimer = null; saveLabelLogoCache(); }, 10000);
}
...
function seedLabelsFromCache() {
  for (const al of albumIndex.albums) {
    ...
    if (q && q.label) {
      ...
      labelDiskCache.set(key, q.label);
    }
  }
  labelsIndex.count = labelsIndex.map.size;
    ...
    try {
      ...
      if (label) {
        ...
        if (gk && !labelMbidCache.has(gk)) {
          ...
          if (resolvedMbid) {
            labelMbidCache.set(gk, resolvedMbid);
...
async function fetchFanArtLogo(groupKey, mbid) {
  ...
  try {
    ...
    if (e.message && e.message.includes("404")) {
      labelLogoCache.set(groupKey, null);
      scheduleLogoSave();
    }
...
async function kickFanArtFetches() {
  ...
  for (const [groupKey, entry] of labelsIndex.map) {
    ...
    if (labelLogoCache.has(groupKey)) continue; // already attempted
    await fetchFanArtLogo(groupKey, entry.mbid);
    await new Promise(r => setTimeout(r, 500)); // polite gap between requests
  }
}
...
async function fetchQobuz(title, artist) {
  ...
  if (out && out.label && !labelDiskCache.has(key)) {
    labelDiskCache.set(key, out.label);
    scheduleDiskSave();
...
function persistRadio() {
  try { roon.save_config && roon.save_config("rra_settings", { radioZones: [...radioZones] }); } catch (e) {}
}
...
// ---------------------------------------------------------------------------
// External streaming links — Spotify search URL (no key), Apple Music exact
// link via iTunes Search API (free, no key), Last.fm constructed URL.
// Two flavours: album (entity=album → collectionViewUrl) and track (entity=song
// → trackViewUrl).  The track variant is used by the now-playing screen so that
// entrypoint sources (Spotify via roon-entrypoints, AirPlay, etc.) also get
// useful "open in" deep-links even though Roon only exposes display strings.
// ---------------------------------------------------------------------------
async function fetchExternalLinks(title, artist) {
  const q = [title, artist].filter(Boolean).join(" ");
  const links = {
    spotify:     `https://open.spotify.com/search/${encodeURIComponent(q)}`,
    lastfm:      `https://www.last.fm/music/${encodeURIComponent(artist || "")}/_/${encodeURIComponent(title || "")}`,
    apple_music: null
  };
  if (!q) return links;
  try {
    const json = await httpJson(
      `https://itunes.apple.com/search?term=${encodeURIComponent(q)}&entity=album&limit=5&media=music`,
      { "User-Agent": MB_USER_AGENT },
      6000
    );
    const results = (json && json.results) || [];
    ...
    const match = results.find(r =>
      r.collectionViewUrl && r.wrapperType === "collection" &&
      (!nArtist || normalize(r.artistName || "").includes(nArtist.split(" ")[0]))
    ) || results.find(r => r.collectionViewUrl);
    if (match) links.apple_music = match.collectionViewUrl;
  } catch (e) {
    if (DEBUG) console.error("[external:itunes]", e.message);
  }
  return links;
}

// Track-level external links — used by the now-playing screen.  Works for both
// local library tracks and entrypoint (audio-input) sources such as Spotify
// via roon-entrypoints: Roon exposes only display text (line1/line2/line3) for
// those, so we derive streaming links from the track + artist name.
//
// Resolution order for Spotify:
//   1. MusicBrainz recording search → url-rels → exact open.spotify.com/track URL
//   2. Fallback: open.spotify.com/search URL (works even without a match)
// Apple Music: iTunes Search API (entity=song) → trackViewUrl
// Last.fm: constructed URL always; optional track.getInfo data if LASTFM_API_KEY is set.

const LASTFM_KEY = process.env.LASTFM_API_KEY || "";
const mbTrackCache = new Map(); // keyed by "track||artist", short-lived

async function fetchMbSpotifyUrl(track, artist) {
  if (!track) return null;
  const cacheKey = normalize(track) + "||" + normalize(artist || "");
  if (mbTrackCache.has(cacheKey)) return mbTrackCache.get(cacheKey);

  try {
    // Step 1: MusicBrainz recording search
    await mbWait();
    let q = `recording:"${mbQuote(track)}"`;
    if (artist) q += ` AND artistname:"${mbQuote(artist)}"`;
    const searchResult = await httpJson(
      `https://musicbrainz.org/ws/2/recording?query=${encodeURIComponent(q)}&limit=3&fmt=json`,
      { "User-Agent": MB_USER_AGENT },
      8000
    );
    const recordings = (searchResult && searchResult.recordings) || [];
    // Only trust high-confidence results (MB scores 0–100)
    const best = recordings.find(r => (r.score || 0) >= 88);
    if (!best || !best.id) { mbTrackCache.set(cacheKey, null); return null; }

    // Step 2: fetch URL relationships for this recording MBID
    await mbWait();
    const recResult = await httpJson(
      `https://musicbrainz.org/ws/2/recording/${best.id}?inc=url-rels&fmt=json`,
      { "User-Agent": MB_USER_AGENT },
      8000
    );
    const rels = (recResult && recResult.relations) || [];
    const spotifyRel = rels.find(r =>
      r.url && r.url.resource && r.url.resource.includes("open.spotify.com/track/")
    );
    const url = spotifyRel ? spotifyRel.url.resource : null;
    // Cache for 10 minutes
    mbTrackCache.set(cacheKey, url);
    setTimeout(() => mbTrackCache.delete(cacheKey), 10 * 60 * 1000);
    return url;
  } catch (e) {
    if (DEBUG) console.error("[mb:spotify-track]", e.message);
    mbTrackCache.set(cacheKey, null);
    return null;
  }
}

async function fetchLastFmTrackData(track, artist) {
  if (!LASTFM_KEY || !track) return null;
  try {
    const url =
    ...
    const t = json && json.track;
    if (!t) return null;
    return {
      listeners: parseInt(t.listeners, 10) || null,
      playcount: parseInt(t.playcount,  10) || null,
      url:       t.url || null,
      tags:      ((t.toptags && t.toptags.tag) || []).slice(0, 5).map(g => g.name).filter(Boolean)
    };
  } catch (e) {
    if (DEBUG) console.error("[lastfm:track]", e.message);
    return null;
  }
}

app.get("/api/track/external-links", async (req, res) => {
  const track  = String(req.query.track  || "");
  const artist = String(req.query.artist || "");
  const album  = String(req.query.album  || "");
  if (!track) return res.status(400).json({ error: "track query parameter required" });

  const q = [track, artist].filter(Boolean).join(" ");

  // All three lookups run in parallel where possible.
  // MusicBrainz requires two sequential requests internally but shares the
  // global mbWait() rate-limiter with the rest of the extension.
  const [mbSpotify, itunesUrl, lastfmData] = await Promise.all([
    fetchMbSpotifyUrl(track, artist),
    (async () => {
      try {
        const json = await httpJson(
          `https://itunes.apple.com/search?term=${encodeURIComponent(q)}&entity=song&limit=5&media=music`,
          { "User-Agent": MB_USER_AGENT }, 6000
        );
        const results = (json && json.results) || [];
        const nArtist = artist ? normalize(artist) : "";
        return (
          results.find(r =>
            r.trackViewUrl && r.wrapperType === "track" &&
            (!nArtist || normalize(r.artistName || "").includes(nArtist.split(" ")[0]))
          ) ||
    ...
    LASTFM_KEY ? fetchLastFmTrackData(track, artist) : null
  ]);

  res.json({
    spotify:       mbSpotify || `https://open.spotify.com/search/${encodeURIComponent(q)}`,
    spotify_exact: !!mbSpotify,
    apple_music:   itunesUrl,
    lastfm:        `https://www.last.fm/music/${encodeURIComponent(artist)}/_/${encodeURIComponent(track)}`,
    lastfm_data:   lastfmData
  });
});

// Album metadata extras: release year (MusicBrainz) + bios (Qobuz/Wikipedia)
// + external streaming links (iTunes → Apple Music, Spotify search, Last.fm).
...
app.get("/api/album/extras", async (req, res) => {
  ...
  try {
    const [year, bios, links] = await Promise.all([
      fetchAlbumYear(title, artist),
      fetchAlbumBios(title, artist),
      fetchExternalLinks(title, artist)
    ]);
...
app.get("/api/zone-state", (req, res) => {
  ...
  res.json({
    zone: {
      ...
      // Heuristic: audio-input (entrypoint) zones playing via Spotify/AirPlay/
      // etc. typically have no seek, no prev, and no next — the external app owns
      // the queue.  We surface this so the UI can show a "Live stream" badge and
      // offer streaming service deep-links even for non-library sources.
      is_audio_input: zone.state === "playing" &&
                      !zone.is_seek_allowed &&
...
app.post("/internal/relay-event", express.json(), (req, res) => {
  ...
  const { zone_id, ...evt } = req.body || {};
  if (!zone_id) return;
  const relay = relays.get(zone_id);
  if (relay) relay.handleEvent(evt);
});

// GET /api/relay/status — relay state + librespot availability
app.get("/api/relay/status", (req, res) => {
  const allZones = Object.values(zones).map(z => ({
    zone_id:    z.zone_id,
    zone_name:  z.display_name
  }));
  const relayList = {};
  for (const [zid, relay] of relays) relayList[zid] = relay.toJSON();
  res.json({
    librespot_found: !!librespotPath,
    librespot_path:  librespotPath,
    relays:          relayList,
    all_zones:       allZones
  });
});

// POST /api/relay/start  body: { zone_id }
app.post("/api/relay/start", async (req, res) => {
  if (!core)        return res.status(503).json({ error: "Not paired with Roon Core" });
  if (!audioinput)  return res.status(503).json({ error: "AudioInput service unavailable" });
  if (!librespotPath) return res.status(400).json({ error: "librespot not found — see setup instructions" });

  const zoneId = String(req.body?.zone_id || "");
  const zone   = zoneId && zones[zoneId];
  if (!zone) return res.status(404).json({ error: "Zone not found" });
  if (relays.has(zoneId)) return res.json({ ok: true, already: true });

  const relay = new ZoneRelay(zoneId, zone.display_name, PORT, DEBUG);
  relays.set(zoneId, relay);

  relay.on("stopped", () => { if (relays.get(zoneId) === relay) relays.delete(zoneId); });
  relay.on("error",   () => { /* state stored in relay.error */ });

  res.json({ ok: true }); // respond immediately; start() is async
  relay.start(librespotPath, audioinput).catch(() => {});
});

// POST /api/relay/stop  body: { zone_id }
app.post("/api/relay/stop", async (req, res) => {
  const zoneId = String(req.body?.zone_id || "");
  const relay  = relays.get(zoneId);
  if (!relay) return res.json({ ok: true, already: true });
  await relay.stop();
