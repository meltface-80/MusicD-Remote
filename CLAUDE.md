...
- Current stable version in the README: **v1.7.72** (until the user says otherwise).
...
| v1.7.71 | superseded (release deleted upstream) | Album header matched to the Android build: back chevron top-left, Share owning the top-right, the ⋯ menu computed from the control height instead of two magic numbers — and given the safe-area inset it never had, so it stopped being a button that did nothing under the iOS status bar. Roon Radio moved to Settings → Playback beside Random album radio (mutually exclusive, clearer together); a vacuous transport-row layout assertion found and fixed; 701 unit / 354 DOM / 70 static — README points here |
| v1.7.72 | **Latest (stable)** | The dial as a second installable page at `/dial` — a swept volume ring, cover in the middle, transport under it, ported literally from the Android build's DialView (320° full range, quantised to the output's own step, sub-step residual carried). `/api/volume` gained `relative_step`; muted state read from the output not the volume object; voice deliberately not ported (dead in an installed iOS web app); 701 unit / 366 DOM / 74 static — README points here |
