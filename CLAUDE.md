...
- Current stable version in the README: **v1.7.70** (until the user says otherwise).
...
| v1.7.71 | withdrawn (release and tag deleted) | Album header matched to the Android build: back chevron top-left, Share owning the top-right, the ⋯ menu computed from the control height instead of two magic numbers — and given the safe-area inset it never had, so it stopped being a button that did nothing under the iOS status bar. Roon Radio moved to Settings → Playback beside Random album radio (mutually exclusive, clearer together); a vacuous transport-row layout assertion found and fixed. The code is still on `main`; no release carries it |
| v1.7.72 | withdrawn (release and tag deleted) | The dial as a second installable page at `/dial`. Withdrawn along with v1.7.71 — the code is still on `main`, the docs no longer mention it, and no release carries it |
